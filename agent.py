#
#
# Agent class is imported by the main Jupyter notebook.
#
#
import random
...
class Agent():
    ...
    def __init__(self, state_size, action_size, seed):   #last arg was , , use_dueling=False use_double=False
...
class ReplayBuffer:
    """A buffer of a fixed-size which stores experience tuples."""
    ...
    def add(self, state, action, reward, next_state, done):
        ...
        new_experience = self.experience(state, np.int32(action), reward, next_state, done)
        self.memory.append(new_experience)

    def sample(self):
        """Randomly sample a batch of experiences from the agent's memory."""
        experiences = random.sample(self.memory, k=self.batch_size)

        states = torch.from_numpy(np.vstack([e.state for e in experiences if e is not None])).float().to(device)
        actions = torch.from_numpy(np.vstack([e.action for e in experiences if e is not None])).long().to(device) #long()
